...
class MergeLayer1(nn.Cell):
    ...
    def construct(self, list_x, x_size):
        ...
        # layer6 -> layer0
        up_sal.append(ops.interpolate(self.score[num_f - 1](tmp), x_size, align_corners=True))
        ...
        for j in range(2, num_f):
            ...
            if list_x[i].shape[1] < u_tmp.shape[1]:
                u_tmp = list_x[i] + ops.interpolate((self.trans[i](u_tmp)), list_x[i].shape[2:],
                                                    align_corners=True)
            else:
                u_tmp = list_x[i] + ops.interpolate(u_tmp, list_x[i].shape[2:], align_corners=True)
            # Conv
            tmp = self.up[i](u_tmp)
            u_tmp = tmp
            sal_feature.append(tmp)
            up_sal.append(ops.interpolate(self.score[i](tmp), x_size, align_corners=True))

        u_tmp = list_x[0] + ops.interpolate(self.trans[-1](sal_feature[0]), list_x[0].shape[2:],
                                            align_corners=True)
        tmp = self.up[0](u_tmp)
        # layer 2
        edge_feature.append(tmp)
        up_edge.append(ops.interpolate(self.score[0](tmp), x_size, align_corners=True))
    ...
    def construct(self, list_x, list_y, x_size):
        ...
        for i, i_x in enumerate(list_x):
            for j, j_x in enumerate(list_y):
                tmp = ops.interpolate(self.trans[i][j](j_x), i_x.shape[2:], align_corners=True) + i_x
                tmp_f = self.up[i][j](tmp)
                up_score.append(ops.interpolate(self.score[i][j](tmp_f), x_size, align_corners=True))
                tmp_feature.append(tmp_f)

        tmp_fea = tmp_feature[0]
        for i_fea in range(len(tmp_feature) - 1):
            tmp_fea = self.relu(tmp_fea + ops.interpolate(tmp_feature[i_fea + 1], tmp_feature[0].shape[2:],
                                                               align_corners=True))
        up_score.append(ops.interpolate(self.final_score(tmp_fea), x_size, align_corners=True))
        return up_score
